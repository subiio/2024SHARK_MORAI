...
class TrackerNode:
    def __init__(self):
        ...
        # self.classes = rospy.get_param("~classes", [0,1,2,16,17]) # car 포함
        self.classes = rospy.get_param("~classes", [0,1,16,17]) # car 포함x
        ...
        self.results_pub = rospy.Publisher(self.result_topic, YoloResult, queue_size=10)
        self.result_image_pub = rospy.Publisher(
            self.result_image_topic, Image, queue_size=10
        )
        # self.detect_pub = rospy.Publisher("/bbox", Detection2DArray, queue_size=10)
    ...
    def image_callback(self, msg):
        ...
        # rospy.loginfo(f"Image Time: {self.rostime}")
        ...
        if results is not None:
            ...
            # self.detection = yolo_result_msg.detections 
            # self.detect_pub.publish(self.detection)

            yolo_result_image_msg = self.create_result_image(results)
            if self.use_segmentation:
                yolo_result_msg.masks = self.create_segmentation_masks(results)
            yolo_result_msg.header.stamp = self.rostime
            self.results_pub.publish(yolo_result_msg)
            self.result_image_pub.publish(yolo_result_image_msg)

    def create_detections_array(self, results):
        detections_msg = Detection2DArray()
        detections_msg.header.stamp = self.rostime  # Detection2DArray 메시지의 헤더 설정
        bounding_box = results[0].boxes.xywh
        classes = results[0].boxes.cls
        confidence_score = results[0].boxes.conf
        for bbox, cls, conf in zip(bounding_box, classes, confidence_score):
            detection = Detection2D()
            detection.header.stamp = self.rostime  # 각 Detection2D 메시지의 헤더 설정
