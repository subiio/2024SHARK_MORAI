...
class Parameter:
    ...
    speed_max = 15.0 / 3.6  # maximum speed [m/s]
    speed_min = -15.0 / 3.6  # minimum speed [m/s]
...
def normalize_angle_degrees(angle):
    ...
    while angle < -180:
        angle += 360
    return angle
def mpc_pure_pursuit(node, ego_ind, global_path_x, global_path_y, mission,flag = 0):
    ...
    i = 0 # v*dt = 거리 >> 내 x + 거리 = > 인덱스 찾아서 그 때의 스티어를 구하자
    steer_list = []
    print("global_path:", global_path_x, global_path_y)
    for i in range(Parameter.T + 1):
...
class PathVisualizer:
    def __init__(self):
        ...
        self.pose_array_pub = rospy.Publisher('/frenet_path', PoseArray, queue_size=10)
        
    def publish_path(self, path):
        pose_array = PoseArray()
        pose_array.header = Header()
        pose_array.header.stamp = rospy.Time.now()
        pose_array.header.frame_id = "map"  # Make sure this matches your RViz fixed frame

        for i in range(len(path.x)):
            pose = Pose()
            pose.position.x = path.x[i]
            pose.position.y = path.y[i]
            pose.position.z = 0  # Assuming a 2D plane
            
            # Convert yaw to quaternion for orientation
            quaternion = quaternion_from_euler(0, 0, path.yaw[i])
...
class MPC:
    ...
    def update_flag(self, ego_ind):
        if 3314 <= ego_ind <= 4200:
            if not self.Parking_mission and abs(ego_ind - 3314) < 20 and self.count == 0:
                ...
                self.count += 1
            # elif self.e_stop == True:
            #     self.flag = 2

            else:
                self.flag = 1
        elif 1500 <= ego_ind <= 2000:
            self.flag = 2
            ...
            # if self.e_stop == True:
            #     self.flag = 2
            #     print("stooooooooooooooooooooooooooooooooooooooop")
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.flag == 0 and self.Parking_mission == False and self.Parking_mission2 == False:
                ...
                input_speed,input_steer,input_break, z_ref, selected_index = self.Model_Predictive_Control(ego_ind)
                input_speed = 15
            elif (self.flag == 0 or self.flag == 1) and self.Parking_mission == True:
                print("Parkinging~~~~~~~~~~~")
                ...
                if self.node.y >= (105.9 + 5) and self.ret:
                    ...
                    self.Parking_mission2 = False
            # elif self.flag == 2:
            #     print("e-stooooooooooooooooooooooooooooooooooooooooop")
            #     input_speed = 0
            #     input_steer = 0

            elif self.flag == 2 and self.Parking_mission == False and self.Parking_mission2 == False:
                # print("ego_ind: ", ego_ind)
                if self.local_count == 0:
                    self.cx = self.local_path[0]
                    self.cy = self.local_path[1]
                    self.cyaw = self.local_path[2]
                    self.local_count = 1

                ego_ind = self.nearest_index(self.node)
                path = optimal_frenet_frame.main(self.cx[ego_ind:ego_ind + 60], self.cy[ego_ind : ego_ind + 60])
                visualizer = PathVisualizer()
                visualizer.publish_path(path)
                self.cx = path.x
                self.cy = path.y
                self.cyaw = path.yaw
                # print("yaw:", path.yaw, "x: ", path.x)
                self.ref_path = PATH(path.x,path.y, path.yaw, self.ck)
                # print("self.ref_path:", self.ref_path.cx, self.ref_path.cyaw)
                input_speed,input_steer,input_break, z_ref, selected_index = self.Model_Predictive_Control(0)
                print("input_speed: ", input_speed, input_steer)
    ...
    def Model_Predictive_Control(self, ego_ind):
        ...
        steer_list = mpc_pure_pursuit(self.node, ego_ind, self.cx, self.cy, self.Parking_mission, self.flag) # 내 위치에서 뽑아낸 미래 예측 steer들
        # print(steer_list)
